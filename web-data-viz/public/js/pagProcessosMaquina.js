    ...
    for (var i = 0; i < resposta.length; i++) {
        ...
        table_processos.innerHTML += `
                <tr>
                    ...
                    <td class="status">${registro.status}</td>
    ...
    fetch(`/info`, {
        ...
        if (response.ok) {
            response.json().then(function (resposta) {
                ...
                sessionStorage.HOST_NAME = resposta[0].hostName;
                sessionStorage.SO = resposta[0].sistemaOperacional;
                sessionStorage.SECAO = resposta[0].secao;
                div_nome_maquina.innerHTML = sessionStorage.HOST_NAME
                div_sistema_operacional.innerHTML = sessionStorage.SO
                div_secao.innerHTML = sessionStorage.SECAO
            });
        } else {
            console.error('Nenhum dado encontrado ou erro na API');
        }
